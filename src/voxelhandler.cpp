...
namespace EITS
{
	VoxModel::VoxModel(void) :width(2), height(2), depth(2), scale(1), size(1, 1, 1), is_loaded(false), is_view_data(true)
	{
		this->is_milling = false;
		this->is_affine = true;
		this->is_view_slice = false;
		this->free_label = 0;
		this->num_voxel = this->width*this->height*this->depth;
		this->length = 1;
		this->newVoxel();
	...
	{
		if (!this->is_loaded || !this->is_view_data)
			return;
	...
	void VoxModel::setSlice(bool _invert)
	{
		...
		for (int i = 0;i<this->width;i++) {
			for (int j = 0;j<this->width;j++) {
				if (!_invert) {
					this->slice[0].setColorAt(i, j, Vector3d(this->getColorAt(this->slice_pos.x, i, j)*255.0).X);
					this->slice[1].setColorAt(i, j, Vector3d(this->getColorAt(i, this->slice_pos.y, j)*255.0).X);
					this->slice[2].setColorAt(i, j, Vector3d(this->getColorAt(i, j, this->slice_pos.z)*255.0).X);
				}
				else {
					Vector3d t_color;
					t_color.x = Vector3d(this->getColorAt(this->slice_pos.x, i, j)*255.0).z;
					t_color.y = Vector3d(this->getColorAt(this->slice_pos.x, i, j)*255.0).y;
					t_color.z = Vector3d(this->getColorAt(this->slice_pos.x, i, j)*255.0).x;
					this->slice[0].setColorAt(i, j, t_color.X);
					t_color.x = Vector3d(this->getColorAt(i, this->slice_pos.y, j)*255.0).z;
					t_color.y = Vector3d(this->getColorAt(i, this->slice_pos.y, j)*255.0).y;
					t_color.z = Vector3d(this->getColorAt(i, this->slice_pos.y, j)*255.0).x;
					this->slice[1].setColorAt(i, j, t_color.X);
					t_color.x = Vector3d(this->getColorAt(i, j, this->slice_pos.z)*255.0).z;
					t_color.y = Vector3d(this->getColorAt(i, j, this->slice_pos.z)*255.0).y;
					t_color.z = Vector3d(this->getColorAt(i, j, this->slice_pos.z)*255.0).x;
	...
	void VoxModel::renderSlicer()
	{
		...
		glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), -this->size.x*width*length / 2, -this->size.x*width*length / 2);
		glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), this->size.x*width*length / 2, -this->size.x*width*length / 2);
		glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), this->size.x*width*length / 2, this->size.x*width*length / 2);
		glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), -this->size.x*width*length / 2, this->size.x*width*length / 2);
		glEnd();
		//glBegin(GL_LINES);
		//glVertex3d(this->size.x*this->length*(slice_pos.x-width/2+1), this->size.x*this->length*(slice_pos.y-width/2+1), -this->size.x*width*length/2);
		//glVertex3d(this->size.x*this->length*(slice_pos.x-width/2+1), this->size.x*this->length*(slice_pos.y-width/2+1),  this->size.x*width*length/2);
		//glEnd();

		glColor3d(0, 1, 0);
		glBegin(GL_LINE_LOOP);
		glVertex3d(-this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), -this->size.x*width*length / 2);
		glVertex3d(this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), -this->size.x*width*length / 2);
		glVertex3d(this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), this->size.x*width*length / 2);
		glVertex3d(-this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), this->size.x*width*length / 2);
		glEnd();
		//glBegin(GL_LINES);
		//glVertex3d(-this->size.x*width*length/2, this->size.x*this->length*(slice_pos.y-width/2+1),  this->size.x*this->length*(slice_pos.z-width/2+1));
		//glVertex3d( this->size.x*width*length/2, this->size.x*this->length*(slice_pos.y-width/2+1),  this->size.x*this->length*(slice_pos.z-width/2+1));
		//glEnd();

		glColor3d(0, 0, 1);
		glBegin(GL_LINE_LOOP);
		glVertex3d(-this->size.x*width*length / 2, -this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glVertex3d(this->size.x*width*length / 2, -this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glVertex3d(this->size.x*width*length / 2, this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glVertex3d(-this->size.x*width*length / 2, this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glEnd();
		//glBegin(GL_LINES);
		//glVertex3d(this->size.x*this->length*(slice_pos.x-width/2+1), -this->size.x*width*length/2,  this->size.x*this->length*(slice_pos.z-width/2+1));
		//glVertex3d(this->size.x*this->length*(slice_pos.x-width/2+1),  this->size.x*width*length/2,  this->size.x*this->length*(slice_pos.z-width/2+1));
		//glEnd();
		...
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0, 0.0);glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), -this->size.x*width*length / 2, -this->size.x*width*length / 2);
		glTexCoord2f(1.0, 0.0);glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), this->size.x*width*length / 2, -this->size.x*width*length / 2);
		glTexCoord2f(1.0, 1.0);glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), this->size.x*width*length / 2, this->size.x*width*length / 2);
		glTexCoord2f(0.0, 1.0);glVertex3d(this->size.x*this->length*(slice_pos.x - width / 2), -this->size.x*width*length / 2, this->size.x*width*length / 2);
		glEnd();
		...
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0, 0.0);glVertex3d(-this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), -this->size.x*width*length / 2);
		glTexCoord2f(1.0, 0.0);glVertex3d(this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), -this->size.x*width*length / 2);
		glTexCoord2f(1.0, 1.0);glVertex3d(this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), this->size.x*width*length / 2);
		glTexCoord2f(0.0, 1.0);glVertex3d(-this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.y - width / 2), this->size.x*width*length / 2);
		glEnd();
		glBindTexture(GL_TEXTURE_2D, 0);
		...
		glBegin(GL_POLYGON);
		glTexCoord2f(0.0, 0.0);glVertex3d(-this->size.x*width*length / 2, -this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glTexCoord2f(1.0, 0.0);glVertex3d(this->size.x*width*length / 2, -this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glTexCoord2f(1.0, 1.0);glVertex3d(this->size.x*width*length / 2, this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glTexCoord2f(0.0, 1.0);glVertex3d(-this->size.x*width*length / 2, this->size.x*width*length / 2, this->size.x*this->length*(slice_pos.z - width / 2));
		glEnd();
		glBindTexture(GL_TEXTURE_2D, 0);
	...
	bool VoxModel::saveSlice(const char *_filename, int _axis)
	{
		...
		Vector3i t_pos = this->slice_pos;
		char t_filename[256];
		bool ret = true;
		for (int d = 0;d<depth;d++) {
			if (_axis == X_AXIS)
				this->setSlicePos(Vector3i(d, 0, 0));
			if (_axis == Y_AXIS)
				this->setSlicePos(Vector3i(0, d, 0));
			if (_axis == Z_AXIS)
				this->setSlicePos(Vector3i(0, 0, d));
			this->setSlice(true);
			sprintf_s(t_filename, "%s%03d.bmp", _filename, d);
	...
	int VoxModel::setNeiborColorWith(Vector3d _tColor, Vector3d _nColor, Vector3d _cColor)
	{
		...
		int x, y, z;
		int nx, ny, nz;
		for (int i = 0;i<this->num_voxel;i++) {
			z = i%depth;
	...
	{
		int *LUT = new int[num_voxel];//ルックアップテーブル
		LUT[0] = 0;
		int *newLUT = new int[num_voxel];//ルックアップテーブル
		int *label = new int[num_voxel];//ラベルを用意する
		int tlabel = 1;
		memset(label, -1, sizeof(int)*num_voxel);//ラベルの値を初期化する
				...
				if (this->data[x][y][z].color.abs() != 0 && this->data[x][y][z].color != INVIS_COL) {
					if (isFacesSandPoint(_point1, _point2, data[x][y][z].center)) {
						if (newLUT[label[i]] == newLUT[tlabel]) {
							...
							this->data[x][y][z].label_index = _label;
						}
					}
				}
			}
	...
	int VoxModel::setLabel()
	{
		...
		for (int i = 0;i < this->num_voxel;i++) {
			...
			y = (int)(i / depth) % height;
			x = (int)(i / depth) / height;
			if (!data[x][y][z].is_surface) {//サーフェースでない領域が見つかった
	...
	void VoxModel::resetColorAt(int _index, Vector3d _color, bool _is_free_space)
	{
		int x, y, z;
		z = _index%depth;
		y = (int)(_index / depth) % height;
		x = (int)(_index / depth) / height;
		free_label = data[x][y][z].label;
		for (int i = 0;i<num_voxel;i++) {
			z = i%depth;
			y = (int)(i / depth) % height;
			x = (int)(i / depth) / height;
			if (data[x][y][z].label == free_label) {
				data[x][y][z].color = _color;
				data[x][y][z].is_free_space = _is_free_space;
			}
		}
	}
...
//		std::cout << data[ix][iy][iz].label << std::endl;
		for (int i = 0;i<num_voxel;i++) {
			z = i%depth;
			y = (int)(i / depth) % height;
			x = (int)(i / depth) / height;
			if (data[x][y][z].label != data[ix][iy][iz].label) {
				data[x][y][z].is_free_space = false;
				if (data[x][y][z].is_surface) {
					data[x][y][z].color = _sColor;
				}
				else{
					data[x][y][z].color = _iColor;
					//std::cout << _sColor << std::endl;
				}
			}
			else {
				data[x][y][z].is_free_space = true;
	...
	void VoxModel::setGradMapWithinSurf()
	{
		...
		do {
			...
			target += SURF_COL;
									...
									//色をスケーリングする
		colormap.setParam(1, numRep+1);
		int x, y, z;
		for (int i = 0;i<num_voxel;i++) {
			z = i%depth;
			y = (int)(i / depth) % height;
			x = (int)(i / depth) / height;
			if (data[x][y][z].color != FREE_COL)
				data[x][y][z].color = colormap.getColorMap(data[x][y][z].color.z, MAP_JET);
		}
		//刺激の階層がいくらか
		this->map_depth = numRep;
	}
	...
	bool VoxModel::millingSurfaceWith(int _index)
	{
		...
		ix = (int)(_index / depth) / height;
		bool isMilled = false;
		if (!this->is_loaded)return isMilled;
		if (_index >= 0) {
			if (!data[ix][iy][iz].is_free_space) {
				data[ix][iy][iz].color = FREE_COL;
				data[ix][iy][iz].is_free_space = true;
				data[ix][iy][iz].is_surface = false;
				data[ix][iy][iz].label = free_label;
				for (int i = 0;i<27;i++) {
					nz = data[ix][iy][iz].neibor[i] % depth;
					ny = (int)(data[ix][iy][iz].neibor[i] / depth) % height;
					nx = (int)(data[ix][iy][iz].neibor[i] / depth) / height;
					if (i != 13 && !data[nx][ny][nz].is_free_space) {
						data[nx][ny][nz].is_surface = true;
	...
	void VoxModel::render(bool _is_scaled)
	{
		if(!this->is_loaded||!this->is_view_data)
			return;
	...
	//	glDisable(GL_DEPTH_TEST);
		...
		for(int x=0;x<width;x++){
			for(int y=0;y<height;y++){
				for(int z=0;z<depth;z++){
					//アルファブレンドはZデプスソートを行う必要がある
					if(this->data[x][y][z].color.abs()!=0&&this->data[x][y][z].color!=INVIS_COL){
						mat.disable();
						glColor4d(this->data[x][y][z].color.x, this->data[x][y][z].color.y, this->data[x][y][z].color.z, 0.1);
						glPushMatrix();
						glTranslated(this->data[x][y][z].center.x,this->data[x][y][z].center.y,this->data[x][y][z].center.z);
						glutSolidRectangle(this->size);
						glPopMatrix();
					}
				}
	...
	int VoxModel::setLabel()
	{
		...
		for(int i=0;i<this->num_voxel;i++){
			...
			y=(int)(i/depth)%height;
			x=(int)(i/depth)/height;
			if(data[x][y][z].is_surface){//サーフェースはすべてラベル0にする
				data[x][y][z].label=0;
			}
