...
namespace EITS{
	...
	bool ColorMap::load(const char *_filename)
	{
		...
		for(int i=0;i<255;i++){
			file >> buf;
			sscanf_s(buf,"%lf,%lf,%lf",&colormap[i].x,&colormap[i].y,&colormap[i].z);
		}
		file.close();
	...
	{
		double t_value = rate * (_value - min) / (max - min);
	...
	int* ColorMap::getColorMapip(double _value){
		...
		if(index<0)index=0;
		currentColor=colormap[index];
		rgb[0]= (int)(currentColor.x*255);
		rgb[1]= (int)(currentColor.y*255);
		rgb[2]= (int)(currentColor.z*255);
		return rgb;
	}
